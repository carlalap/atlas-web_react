...
class Login extends Component {
  constructor(props) {
    super(props);
    this.state = { isLoggedIn: props.isLoggedIn, email: "", password: "", enableSubmit: false};
  ...
  handleLoginSubmit(event) {
    ...
    this.setState({ isLoggedIn: true }); // Update state when form is submitted
  }
        ...
        <form className={css(styles.form)} onSubmit={this.handleLoginSubmit}>
          <div className={css(styles.labelInputContainer)}>
            ...
            <input 
              ...
              value="Log In"
              className={css(styles.loginInput)}
              disable={!this.state.enableSubmit}
            />
          </div>
        </form>
      </div>
